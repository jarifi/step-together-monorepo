...
@router.get("/user", response_model=List[StepLogResponse])
def read_my_step_logs(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    return step_log_crud.get_step_logs_by_user_id(db, current_user.id) or []
...
def read_step_logs_by_user_id(
    ...
    if getattr(current_user, "role", "user") != "admin" and current_user.id != user_id:
        step_logger.warning(
            f"STEP_LOG READ DENIED | requested_user={user_id} | by_user={current_user.id}"
        )
...
@router.get("/challenge/{challenge_id}/user", response_model=List[StepWeekResponse])
def read_my_step_logs_by_date_range(
    challenge_id: int,
...
def read_step_log(
    ...
    if getattr(current_user, "role", "user") != "admin" and step_log.user_id != current_user.id:
        step_logger.warning(
            f"STEP_LOG READ DENIED | step_log_id={step_log_id} | requested_by={current_user.id}"
        )
        raise HTTPException(status_code=403, detail="Not authorized to view this step log")
...
def update_step_log(
    ...
    if getattr(current_user, "role", "user") == "admin":
        ...
        updated_step_log = step_log_crud.update_step_log(
            db, step_log_id, step_log_data, user_id=current_user.id
        )
...
def delete_step_log(
    ...
    if getattr(current_user, "role", "user") != "admin" and step_log.user_id != current_user.id:
        step_logger.warning(
            f"STEP_LOG DELETE FORBIDDEN | step_log_id={step_log_id} | attempted_by={current_user.id}"
        )
        raise HTTPException(status_code=403, detail="Not authorized to delete this step log")
