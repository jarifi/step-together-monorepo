...
const UserRow = ({ user, onPress, isAdd }) => {
  return (
    <View style={styles.userRow}>
      ...
      >
        <Ionicons name={isAdd ? 'add' : 'remove'} size={18} color="#fff" />
      </Pressable>
    </View>
  );
};
...
export default function UpdateTeamScreen() {
  ...
  const initialMaxMembers = useMemo(() => {
    const raw = Array.isArray(params?.maxMembers) ? params.maxMembers[0] : params?.maxMembers;
    const n = Number(raw);
  ...
  const [teamSizeMode, setTeamSizeMode] = useState('custom');
  ...
  useEffect(() => {
    setName(initialName);
    const presets = [2, 3, 5];
    if (presets.includes(initialMaxMembers)) {
      setTeamSizeMode(String(initialMaxMembers));
    } else {
      setTeamSizeMode('custom');
      setCustomSize(String(initialMaxMembers));
    }
  }, [initialName, initialMaxMembers]);
  ...
  useEffect(() => {
    ...
    getTeamMembers(teamId)
      .then((members) => {
        const safe = Array.isArray(members) ? members.map(normalizeUser) : [];
        setSelectedUsers(safe);
      })
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      <View style={styles.formCard}>
        ...
        <View style={styles.sizeRow}>
          {['2', '3', '5', 'custom'].map((option) => {
            ...
            return (
              <Pressable
                key={option}
                onPress={() => setTeamSizeMode(option)}
                style={({ pressed }) => [
                  styles.sizeChip,
        ...
        <View style={styles.userList}>
          {loadingMembers ? (
            <Text style={styles.emptyText}>Lade Mitglieder...</Text>
            ...
            <Text style={styles.emptyText}>Noch keine Benutzer im Team.</Text>
          )}
        </View>
