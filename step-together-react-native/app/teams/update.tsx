...
export default function UpdateTeamScreen() {
    ...
    const params = useLocalSearchParams<UpdateTeamSearchParams>();
    ...
    const [pendingRemovals, setPendingRemovals] = useState<Set<number | string>>(new Set());
        ...
        try {
            ...
            const msg =
                typeof error === 'object' && error !== null && 'response' in error
                    ...
                    : (error instanceof Error ? error.message : 'Team konnte nicht aktualisiert werden.');
    ...
    const loadMembers = async () => {
        ...
        try {
            ...
            setMembers(Array.isArray(data) ? (data as User[]) : []);
        } catch {
            setMembers([]);
        } finally {
            setLoadingMembers(false);
    ...
    const handleRemove = (member: User) => {
        ...
        if (member.pending) {
            setPendingAdditions((prev) => prev.filter((u) => u.id !== memberId));
            return;
        }
    ...
    const handleAdd = (user: SearchUser) => {
        setPendingAdditions((prev) => [...prev, user as User]);
        setSearchQuery('');
        setSearchResults([]);
    };

    const displayedMembers: User[] = [
        ...members.filter((m) => {
            const memberId = m.id ?? m.userId;
            return memberId != null && !pendingRemovals.has(memberId);
        }),
    ...
    const effectiveMemberUserIds = new Set<unknown>([
        ...members
            .filter((m) => {
                const memberId = m.id ?? m.userId;
                return memberId != null && !pendingRemovals.has(memberId);
    ...
    return (
        <View style={styles.screen}>
            <FlatList
                ...
                keyExtractor={(m) => String(m.id ?? m.userId ?? 'pending')}
                ListHeaderComponent={
                    <View>
            ...
            <View style={styles.footer}>
                ...
                >
                    <Text style={styles.primaryBtnText}>{savingName ? 'Speichern…' : 'Speichern'}</Text>
                </Pressable>
            </View>
        </View>
    );
