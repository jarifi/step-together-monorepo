...
type UpdateHybridChallengeParams = {
    id?: string | string[];
};
...
type ChallengeInvite = {
    status?: string | null;
    inviteeUserId?: number | string | null;
    invitee_user_id?: number | string | null;
    inviteeId?: number | string | null;
    invitee_id?: number | string | null;
};
...
type SelectedIds = Array<number | string>;

function extractErrorMessage(err: unknown): string {
    const payload = (err as { payload?: ErrorPayload })?.payload;
    if (payload) {
    ...
    if (status === 422) return 'Eingabefehler. Bitte alle Felder korrekt ausfüllen.';
    if (status && status >= 500) return 'Serverfehler. Bitte später erneut versuchen.';
    if ((err as { name?: string })?.name === 'TypeError') return 'Netzwerkfehler. Bitte Internetverbindung prüfen.';
                    ...
                    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={cal.scrollContent}>
                        ...
                        <View style={cal.grid}>
                            {cells.map((day, i) => {
                                ...
                                const isSelected = picked !== null && sameDay(cellDate, picked);
    ...
    return (
        <View style={s.fieldWrap}>
            ...
            >
                ...
                {hasVal ? (
                    <TouchableOpacity
                        onPress={(e: any) => { e?.stopPropagation?.(); onChange(null); }}
    ...
    return (
        ...
        >
            ...
            <View style={[s.checkbox, selected && s.checkboxOn]}>
                {selected ? <Text style={s.checkTick}>✓</Text> : null}
            </View>
        </TouchableOpacity>
    );
}
...
function UserRow({ user, selected, onToggle }: UserRowProps) {
    ...
    const displayName =
        (user.name ?? user.fullname ?? user.full_name ?? user.username) || 'Unbekannt';
    ...
    return (
        ...
        >
            ...
            <View style={s.teamBody}>
                ...
                <Text style={s.teamMeta}>ID: {String(user.id)}</Text>
            </View>
            <View style={[s.checkbox, selected && s.checkboxOn]}>
                {selected ? <Text style={s.checkTick}>✓</Text> : null}
            </View>
...
export default function UpdateHybridChallenge() {
    ...
    const [selectedUserIds, setSelectedUserIds] = useState<SelectedIds>([]);
    ...
    const loadUsers = async (q = query): Promise<User[]> => {
    ...
    const loadInitialData = async () => {
        if (!id) {
            showToast('error', 'Keine Challenge-ID gefunden.');
        ...
        try {
            ...
            setStartLoc(challenge?.start_location ?? challenge?.startLocation ?? '');
            setTargetLoc(challenge?.target_location ?? challenge?.targetLocation ?? '');
            setDistance(challenge?.distance != null ? String(challenge.distance) : '');
            const rawStart = challenge?.start_date ?? challenge?.startDate;
            const rawEnd = challenge?.end_date ?? challenge?.endDate;
            ...
            try {
                ...
                const userIds = Array.isArray(invites)
                    ? invites
                        .filter((inv): inv is ChallengeInvite => !!inv)
                        .filter((inv) => {
                            const status = String(inv.status ?? '').toLowerCase();
                            return status !== 'declined' && status !== 'cancelled';
                        })
                        .map((inv) =>
                            inv.inviteeUserId ?? inv.invitee_user_id ?? inv.inviteeId ?? inv.invitee_id
                        )
                        .filter((invitee): invitee is number | string => invitee != null)
                    : [];
                setSelectedUserIds(userIds);
            } catch (inviteErr) {
                console.warn('Could not load challenge invites:', inviteErr);
            }
        } catch (err) {
    ...
    const toggleUser = useCallback((userId?: number | string | null) => {
    ...
    const validate = (): boolean => {
        ...
        if (!distance || Number.isNaN(Number(distance)) || Number(distance) <= 0) e.distance = 'Gültige Distanz eingeben';
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const parsedCreatorId = Number(storedUserId ?? '0');
            ...
            const payload = {
                ...
                mode: 'individual',
                team_ids: [] as Array<number | string>,
                creator_id: Number.isFinite(parsedCreatorId) ? parsedCreatorId : 0,
            };

            const timeoutPromise = new Promise<unknown>((_, reject) =>
                setTimeout(
                    () => reject(new Error('Zeitüberschreitung. Bitte Verbindung prüfen und erneut versuchen.')),
                    10000
                )
            );

            const updated = await Promise.race([updateChallenge(id, payload), timeoutPromise]);

            if (selectedUserIds.length > 0 && updated && typeof updated === 'object' && 'id' in updated) {
                try {
                    await createBulkChallengeInvites((updated as { id: number | string }).id, selectedUserIds);
                } catch (inviteErr) {
                    console.warn('Invites could not be sent:', inviteErr);
                }
            }

            showToast('success', 'Challenge aktualisiert! Hier klicken um sie zu sehen.', () =>
    ...
    const showToast = (type: ToastState['type'], message: string, onPress?: () => void) => {
        if (toastTimer.current) {
            clearTimeout(toastTimer.current);
        }
    ...
    if (initialLoading) {
        return (
            <View style={[s.root, { alignItems: 'center', justifyContent: 'center' }]}>
                ...
                <Text style={{ marginTop: 12, color: T.textMuted }}>Challenge wird geladen…</Text>
            </View>
        );
    }
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            >
                <View style={s.header}>
                    <Text style={s.headerTitle}>Bearbeite deine Challenge</Text>
                </View>
                ...
                >
                    <SectionHead title="Grunddaten" subtitle="Name, Strecke und Distanz bearbeiten" />
                    <Card>
                        <View style={s.fieldWrap}>
                            <Text style={s.label}>
                        ...
                        <View style={IS_MOBILE ? s.col : s.row2}>
                            <View style={[s.fieldWrap, !IS_MOBILE && { flex: 1 }]}>
                                ...
                                />
                                {errors.startLoc ? <Text style={s.errorMsg}>{errors.startLoc}</Text> : null}
                            </View>
                            ...
                            <View style={[s.fieldWrap, !IS_MOBILE && { flex: 1 }]}>
                                ...
                                />
                                {errors.targetLoc ? <Text style={s.errorMsg}>{errors.targetLoc}</Text> : null}
                            </View>
                        </View>
                        ...
                        <View style={s.fieldWrap}>
                            ...
                            {errors.distance ? <Text style={s.errorMsg}>{errors.distance}</Text> : null}
                        </View>
                    </Card>
                    ...
                    <Card>
                        <View style={IS_MOBILE ? s.col : s.row2}>
                            <View style={!IS_MOBILE ? { flex: 1 } : undefined}>
                                <DateField
                                    label="Startdatum"
                                    required
                                    minDate={today()}
                        ...
                        <View style={s.widgetSearchWrap}>
                            <View style={s.searchBox}>
                                ...
                                <TextInput
                                    ...
                                    value={query}
                                    onChangeText={(text) => { setQuery(text); void loadUsers(text); }}
                                />
                            </View>
                        </View>
                        ...
                        {loadingUsers ? (
                            ...
                            <>
                                {users.slice(0, visibleUsers).map((user, i, arr) => (
                                    <View key={String(user.id ?? i)}>
                                        <UserRow
                                            user={user}
                                            selected={user.id != null && selectedUserIds.includes(user.id)}
                                            onToggle={toggleUser}
                                        />
                    ...
                    >
                        {submitting ? (
                            ...
                            <Text style={s.submitText}>Challenge aktualisieren</Text>
                        )}
                    </TouchableOpacity>
