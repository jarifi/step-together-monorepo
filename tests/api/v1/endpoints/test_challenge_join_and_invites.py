...
def test_individual_challenge_invite_accept_flow(client, db_session):
    ...
    friend_token = _login(client, friend.email)
    ...
    invite_response = client.post(
        f"/api/v1/challenges/{challenge.id}/invites",
        json={"inviteeUserId": friend.id},
        headers={"Authorization": f"Bearer {creator_token}"},
    )
    ...
    accept_response = client.post(
        f"/api/v1/challenges/{challenge.id}/invites/{invite_id}/accept",
    ...
    participant = (
        ...
        .filter(
            ...
            ChallengeParticipant.user_id == friend.id,
        )
        .first()
    )
...
def test_team_challenge_join_keeps_team_based_registration(client, db_session):
    ...
    team = Team(name="Alpha Team", creator_id=creator.id)
    db_session.add(team)
    db_session.commit()
    ...
    db_session.add(ChallengeTeam(challenge_id=challenge.id, team_id=team.id))
    db_session.commit()
    ...
    assert join_response.json()["teamId"] == team.id

    team_member = (
        db_session.query(TeamMember)
        .filter(TeamMember.user_id == member.id, TeamMember.team_id == team.id)
        .first()
    )
