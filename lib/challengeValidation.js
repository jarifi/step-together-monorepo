...
export function validateDistance(distance) {
    ...
    if (distance.length > 10) errors.push("Distanz darf nicht länger als 10 Zeichen sein!");

    if (!distanceRegex.test(distanceStr)) {
        errors.push("Distanz enthält ungültige Zeichen!");
    }
    return errors;
}
...
export function validateDate(startDate, endDate) {
    ...
    if (errors.length === 0) {
        ...
        const startValid = start.getFullYear() === sy && start.getMonth() + 1 === sm && start.getDate() === sd;
        const endValid = end.getFullYear() === ey && end.getMonth() + 1 === em && end.getDate() === ed;
        ...
        if (startValid && endValid && end <= start) {
            errors.push("End-Datum muss später also Start-Datum sein!");
        }
    }
    return errors;
}