...
// Public POST for endpoints that don't require authentication (like registration)
export const publicPost = async (path, bodyObj = {}) => {
  if (!API_BASE_URL) throw new Error('Missing API_BASE_URL (expo constants)');
  const url = path.startsWith('http') ? path : buildUrl(path);

  const body = bodyObj instanceof FormData ? bodyObj : JSON.stringify(bodyObj);
  const headers = bodyObj instanceof FormData ? { Accept: 'application/json' } : { 
  ...
  try {
    ...
    if (isAbortError(fetchErr)) {
      console.warn('🟡 [API] public POST request aborted:', url);
    } else {
      console.error('❌ [API] public POST fetch() threw:', fetchErr, 'URL:', url);
    }
    throw fetchErr;
  }
  ...
  if (!res.ok) {
    ...
    err.status = res.status;
    err.payload = payload;
    console.error('🔴 [API] public POST request failed:', err.status, payload);
    throw err;
  }
  return payload;
};
